...
function SlideMedia({
    ...
    // Desktop: contain the full cinematic frame over a blurred fill.
    // Mobile: cover the portrait viewport so the scene fills the screen.
    return (
        <div className="absolute inset-0 overflow-hidden">
            ...
            <div className={`absolute ${cover ? "inset-0" : "inset-[-8%]"} ${ken}`}>
                <Image
                    src={src}
                    alt=""
                    fill
                    ...
                    onLoad={ready}
                    onError={fail}
                    className={cover ? "object-cover object-[center_30%]" : "object-contain object-center"}
...
export function BackgroundSlideshow({ currentPlaylist }: { currentPlaylist?: string }) {
    ...
    // Exact match on playlist name
    const filteredBackgrounds = currentPlaylist 
        ? BACKGROUNDS.filter(bg => bg.playlist === currentPlaylist)
        : BACKGROUNDS;
